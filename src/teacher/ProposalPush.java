...
public class ProposalPush extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		Iterator<?> namecheck = proposaldao.findByThesistitle(propossaltitle).iterator();
		if(namecheck.hasNext()){
			out.print("<div class=\"alert alert-error alertfix\"><button type=\"button\" class=\"close\" data-dismiss=\"alert\">×</button><strong>警告! </strong> 课题名称已经存在，请勿重复添加。</div>");
			return;
		}
		...
		try {
			...
			Management management = (Management)managementdao.findByProperty("main.id", mainid).iterator().next();
			if(proposaldao.count(mainid+"") < management.getLimitnum()){
				proposal.setMain(management.getMain());
				...
				out.print("<div class=\"alert alert-success alertfix\"><button type=\"button\" class=\"close\" data-dismiss=\"alert\">×</button><strong>恭喜! </strong> 课题添加成功。</div>");
